...
- **`gh_stars_delta`** — impossible historically; this collector stores the star count on every observation so a forward-accruing delta can be built later without re-fetching.
  ...
  **Carry-forward (found in the 2B whole-branch review, 2026-08-18):** that forward plan does not work through `observations` as the table is keyed. Its uniqueness is `(source, doc_id, tech_id)` — no week — and `store.upsert_observations` writes with `INSERT OR IGNORE`, so a repository's `amount` is pinned at the star count it had the first time it was ever seen and every later week's higher count is silently dropped. A delta computed from that column would be zero for every repository, forever. Whoever builds `gh_stars_delta` has to decide where the per-week star count lives; re-keying `observations` is not a fix to make in passing, since the current key is exactly what makes a repository count once across the lookback overlap.
