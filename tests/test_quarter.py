...
def test_rendering_a_quarter_also_writes_the_post_cards_and_the_brief(conn, tmp_path):
    """The deliverables the marketing plan lists per report. Generated by the
    pipeline or they do not get made -- the plan assumes two days a quarter of
    the owner's time, and hand-cutting images is how that budget disappears."""
    watchlist = _filings_quarter(conn)
    quarter.render_quarter(conn, "2026-Q2", watchlist, out_dir=tmp_path)
    cards = sorted(path.name for path in (tmp_path / "cards").glob("*.png"))
